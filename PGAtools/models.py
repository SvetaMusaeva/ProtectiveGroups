...
from CGRdb.config import FP_SIZE, DATA_STEREO, DATA_ISOTOPE
...
from pony.orm import PrimaryKey, Optional, Required, Set, Json, db_session
from .config import DEBUG, GroupStatus, FP_DEEP
...
def load_tables(db, schema, cgr_molecule, cgr_reaction):
    class Group(db.Entity):
        _table_ = '%s_group' % schema if DEBUG else (schema, 'group')
        ...
        def analyse(self, reactions):
            ...
            fingerprints = iter(cgr_molecule.get_fingerprints(tmp) if tmp else [])
        ...
        def analyse_db(self, reactions=None):
            if reactions is None:
                ...
                with db_session:
                    group = Group[self.id]
                    report = self.__populate(group, reactions, self.analyse([x.structure for x in reactions]),
                                             check=True)
    ...
    class RawMedia(db.Entity):
        _table_ = '%s_raw_media' % schema if DEBUG else (schema, 'raw_media')
        id = PrimaryKey(int, auto=True)
    ...
    class Media(db.Entity):
        _table_ = '%s_media' % schema if DEBUG else (schema, 'media')
        id = PrimaryKey(int, auto=True)
        name = Required(str, unique=True)
        raw = Set('RawMedia')
        tags = Set('Tag')
    ...
    class Tag(db.Entity):
        _table_ = '%s_tag' % schema if DEBUG else (schema, 'tag')
        id = PrimaryKey(int, auto=True)
        name = Required(str, unique=True)
        medias = Set('Media', table='%s_media_tag' % schema if DEBUG else (schema, 'media_tag'))

    class GroupReaction(db.Entity):
        _table_ = '%s_group_reaction' % schema if DEBUG else (schema, 'group_reaction')
        id = PrimaryKey(int, auto=True)
    ...
    return Group, GroupReaction, RawMedia, Media, Tag
